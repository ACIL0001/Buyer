...
const RootStyle = styled('span')<{
  ...
  const styleOutlined = (color: string) => ({
    ...
    border: `1px solid ${(theme.palette as any)[color].main}`,
  });

  const styleGhost = (color: string) => ({
    color: (theme.palette as any)[color][isLight ? 'dark' : 'light'],
    backgroundColor: alpha((theme.palette as any)[color].main, 0.16),
  });
