...
export default function OffersPage() {
  ...
  const COLUMNS = [
    { id: 'user', label: 'User', alignRight: false, searchable: true, sortable: true },
    { id: 'tel', label: 'Phone', alignRight: false, searchable: true, sortable: true },
    { id: 'bidTitle', label: 'Auction', alignRight: false, searchable: true, sortable: true },
    { id: 'price', label: 'Price', alignRight: false, searchable: false, sortable: true },
    { id: 'status', label: 'Status', alignRight: false, searchable: false, sortable: true },
  ...
  const getOfferStatusChip = (offer: Offer) => {
    ...
    if (isExpired) {
      return <Chip label="Expired" size="small" color="error" variant="outlined" />;
    }
    if (isActive) {
                ...
                <Stack direction="row" spacing={1}>
                  ...
                  {filterTab === 'received' ? (
                    ...
                    <>
                      ...
                      {(row.status || 'PENDING') !== 'PENDING' && (
                        <Chip
                          label={(row.status || 'PENDING') === 'ACCEPTED' ? 'Accepted' : 'Rejected'}
  ...
  const renderContent = () => {
    if (loading) {
      return (
        <Stack alignItems="center" justifyContent="center" sx={{ py: 8 }}>
          ...
          <Typography variant="body1" sx={{ mt: 2 }}>Loading offers...</Typography>
        </Stack>
      );
    }
    ...
    if (filteredOffers.length === 0) {
      return (
        <Stack alignItems="center" justifyContent="center" sx={{ py: 8 }}>
          <Typography variant="h6" color="text.secondary" gutterBottom>
            {filterTab === 'received' ? 'No received offers' : 'No offers made'}
          </Typography>
          <Typography variant="body2" color="text.secondary" textAlign="center">
            {filterTab === 'received' 
  ...
  return (
    <Container maxWidth="xl" sx={{ px: { xs: 1, sm: 2, md: 3 } }}>
        ...
        >
          <Stack direction={{ xs: 'column', sm: 'row' }} alignItems={{ xs: 'stretch', sm: 'center' }} spacing={2}>
            <Typography variant="h4" sx={{ m: 0 }}>
              {filterTab === 'received' ? 'Received Offers' : 'My Offers'}
            </Typography>
      ...
      <Card sx={{ mb: 3, borderRadius: 2 }}>
        ...
        >
          <Tab 
            label={`Received (${offers.filter(offer => offer.user._id !== auth?.user?._id).length})`} 
            value="received" 
          />
          <Tab 
            label={`My Offers (${offers.filter(offer => offer.user._id === auth?.user?._id).length})`} 
            value="my" 
          />
        </Tabs>
      </Card>
